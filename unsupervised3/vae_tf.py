...
class VariationalAutoencoder:
    def __init__(self, D, hidden_layer_sizes):
        ...
        # now build the cost
        if st is None:
            kl = -tf.log(self.stddev) + 0.5 * (
                    self.stddev ** 2 + self.means ** 2) - 0.5
        ...
        self.train_op = tf.train.RMSPropOptimizer(learning_rate=0.001).minimize(
            -self.elbo)
    ...
    def prior_predictive_sample_with_probs(self):
        ...
        return self.sess.run(
            (self.prior_predictive, self.prior_predictive_probs))
