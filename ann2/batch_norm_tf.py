...
from ann2.util import get_normalized_data
...
class ANN(object):
    ...
    def fit(self, X, Y, X_test, y_test, activation=tf.nn.relu,
            learning_rate=1e-3, epochs=15, batch_size=100, print_period=100,
            show_fig=True):
        ...
        # output will be differently for train and test (prediction)
        ...
        for i in range(epochs):
            ...
            for j in range(n_batches):
                ...
                if (j + 1) % print_period == 0:
                    ...
                    print("epoch:", i, "batch:", j, "n_batches:", n_batches,
                          "cost:", c, "acc: %.2f" % acc)

            print("Train acc:", self.score(X, Y), "Test acc:",
...
if __name__ == '__main__':
    ...
    ann.fit(X_train, y_train, X_test, y_test, show_fig=True)

    print("Train acc", ann.score(X_train, y_train))
    print("Test acc", ann.score(X_test, y_test))