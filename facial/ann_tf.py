...
class ANN(object):
    ...
    def fit(self, X, Y, learning_rate=1e-2, mu=0.99, decay=0.999, reg=1e-3, epochs=10, batch_size=100, show_fig=False):
        ...
        for m2 in self.hidden_layer_sizes:
            ...
            self.hiden_layers.append(h)
            M1 = m2
            count += 1

        W, b = init_weight_and_bias(M1, K)  # latest M1 from the for loop
        self.W = tf.Variable(W.astype(np.float32))
        self.b = tf.Variable(b.astype(np.float32))

        # collect params for later use
        self.params = [self.W, self.b]
        for h in self.hiden_layers:
        ...
        cost = tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits_v2(logits=act, labels=tfT)
        ) + rcost
